...
# st.set_page_config(page_title="Lumina AI | Novelist Suite", layout="wide")

# # Custom CSS for that "Luxury" look
# st.markdown("""
#     <style>
#     .main { background-color: #0e1117; }
#     .stMetric { background-color: #1e2130; padding: 15px; border-radius: 10px; border: 1px solid #3e4259; }
#     .stTextArea textarea { border-radius: 10px; border: 1px solid #4a4e69; }
#     .stButton button { background: linear-gradient(45deg, #6a11cb 0%, #2575fc 100%); color: white; border: none; font-weight: bold; }
...
# def login():
#     st.title("🔐 Lumina AI Login")
#     email = st.text_input("Email Address", placeholder="dev@example.com")
#     password = st.text_input("Password", type="password")
#     if st.button("Enter Executive Suite"):
#         # Simple Logic for now: any login works to keep it easy for you!
#         if email and len(password) > 5:
#             st.session_state.authenticated = True
#             st.session_state.user_email = email
#             st.rerun()
#         else:
#             st.error("Please enter a valid email and password (min 6 chars).")

# if not st.session_state.authenticated:
#     login()
#     st.stop()

# # --- 3. THE AI LOGIC (Expanded for Novels) ---
# def analyze_novel(text):
#     system_prompt = (
#         "You are an Elite Literary Agent. Analyze this novel excerpt and return a JSON object. "
#         "Keys required: 'title', 'genre', 'sub_genre', 'summary', 'profanity_level', 'romance_score', "
#         "'horror_score', 'comedy_score', 'themes', 'takeaways'. "
#         "Values for scores should be 1-10. Profanity should be 'Clean', 'Mild', or 'Explicit'."
#     )
...
#     history = supabase.table("book_history").select("*").eq("user_email", st.session_state.user_email).order("created_at", desc=True).execute()
    
#     if history.data:
#         for entry in history.data:
#             with st.expander(f"📖 {entry['book_title']}"):
#                 st.caption(f"Genre: {entry['genre']}")
#                 st.write(entry['summary'][:100] + "...")
#     else:
#         st.info("No books tested yet.")

# # --- 5. MAIN UI ---
# st.title("✨ Shelfie Bot: Novel Analysis")
# st.caption("Professional Manuscript Evaluation & Summarization")
...
#         else:
#             with st.spinner("Decoding literary patterns..."):
#                 data = analyze_novel(user_text)
                
#                 if "error" in data:
#                     st.error("AI is resting. Try again in 1 minute.")
#                 else:
...
#                         "profanity_level": data.get("profanity_level", ""),
...
#         m3.metric("Sub-Genre", d.get('sub_genre'))
        
#         # Genre Scores
#         st.markdown("#### Genre DNA")
#         c1, c2, c3 = st.columns(3)
#         c1.progress(d.get('romance_score', 0)/10, text=f"💘 Romance: {d.get('romance_score')}/10")
#         c2.progress(d.get('horror_score', 0)/10, text=f"👻 Horror: {d.get('horror_score')}/10")
#         c3.progress(d.get('comedy_score', 0)/10, text=f"😂 Comedy: {d.get('comedy_score')}/10")
...
#         st.info(", ".join(d.get('themes', [])))
        
#         st.markdown("### 💡 Core Takeaways")
#         for t in d.get('takeaways', []):
#             st.write(f"✅ {t}")
#     else:
#         st.info("Input a manuscript on the left to generate the intelligence report.")
